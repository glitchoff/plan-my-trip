        ...
        <div className="min-h-screen pt-20 bg-white">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-white/80 backdrop-blur-md rounded-2xl shadow-lg mt-4">
                ...
                <div className="space-y-4">
                    {pastTrips.map((trip) => (
                        <div key={trip.id} className="bg-white rounded-xl shadow-sm overflow-hidden flex flex-col md:flex-row hover:shadow-md transition-shadow">
                            <div className="md:w-48 h-32 md:h-32 relative flex-shrink-0">
                                <img src={trip.image} alt={trip.destination} className="w-full h-full object-cover absolute inset-0" />
                            </div>
                            <div className="p-4 flex-1 flex flex-col justify-between">
                                <div>
                                    ...
                                    <p className="text-gray-500 text-xs mb-1">📅 {trip.date}</p>
                                    <p className="text-gray-500 text-xs">💰 Total Cost: ${trip.totalCost}</p>
                                </div>
                                <div className="mt-2 flex justify-end gap-3">
                                    <button
                                        onClick={() => alert("Review feature coming soon!")}
                                        className="text-gray-600 font-semibold hover:text-gray-900 text-xs"
                                    >
                                        Leave Review ✍️
                                    </button>
                                    <button className="text-blue-600 font-semibold hover:text-blue-800 text-xs">
                                        View Itinerary &rarr;
                                    </button>
                                </div>
                            </div>
