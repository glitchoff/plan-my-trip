    ...
    return (
        <div className="min-h-screen pt-20 bg-base-100">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-base-100/80 backdrop-blur-md rounded-2xl shadow-lg mt-4">
                <h1 className="text-3xl font-bold text-base-content mb-8">My Wishlist</h1>

                {wishlistItems.length === 0 ? (
                    <div className="text-center py-20">
                        <p className="text-base-content/60 text-lg">Your wishlist is empty.</p>
                        <button className="mt-4 text-primary font-medium hover:text-primary/80">
                            Explore Hotels &rarr;
                        </button>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        {wishlistItems.map((item) => (
                            <div key={item.id} className="bg-base-100 rounded-2xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                                <div className="relative h-48">
                                    <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                    <button className="absolute top-4 right-4 bg-base-100/90 p-2 rounded-full text-error hover:bg-base-100 transition-colors">
                                        ♥️
                                    </button>
                                </div>
                                <div className="p-6">
                                    <div className="flex justify-between items-start mb-2">
                                        ...
                                        <span className="bg-primary/20 text-primary text-xs font-semibold px-2.5 py-0.5 rounded">
                                            {item.rating} ★
                                        </span>
                                    </div>
                                    <p className="text-base-content/60 text-sm mb-4">📍 {item.location}</p>
